...
int menu(void);					/* メニュー */
void add(void);					/* データの追加 */
void del(void);					/* データの削除 */
void calc(void);				/* 所要時間の計算 */
void station_disp(void);		/* 路線データを表示 */
int read_data(void);			/* データファイルの読み込み */
int get_num(void);
STATION *check(char target[]);	/* 入力された駅がリストに存在しているかどうかのチェック */
...
void station_disp(void)
{
	...
	while (1)
	{
		...
		current_addr = current_addr->next_addr;	/* current_addrを更新 */
	}
}
...
STATION *check(char target[])
{
	...
	while (1)
	{
		if (!strcmp(target, current_addr->name))	return current_addr;	/* 駅名が一致したら */
		if (current_addr->next_addr == NULL)	break;
		current_addr = current_addr->next_addr;	/* current_addrを更新 */
	}
